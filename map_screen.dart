...
part 'map_screen_models.dart';
part 'map_screen_sheets.dart';
part 'map_screen_search_overlay.dart';
...
class _MapScreenState extends State<MapScreen> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  /// `users.avatar` — animal/personaj din profil; fallback la nametag implicit al aplicației.
  String _nametagEmojiFromProfile() {
    final v = _driverProfile?['avatar'] as String?;
    final t = v?.trim();
    if (t != null && t.isNotEmpty) return t;
    return '🙂';
  }

  /// Ierarhie: **Garaj** (non-default) → **șofer la volan** fără garaj = mașină standard → altfel **emoji** (pasager / șofer off-duty).
  String _ownUserMarkerVisualKey() {
    ...
    if (_driverOnDutyOnMap) {
      return 'driver:standard';
    }
    return 'passenger:emoji:${_nametagEmojiFromProfile()}';
  }
  ...
  Future<void> _syncMapOrientationPinAnnotation() async {
    ...
    unawaited(() async {
      ...
      try {
        ...
        // âœ… DECALAJ DACÄ‚ SE SUPRAPUNE CU UTILIZATORUL/MAÈ˜INUÈšA (pentru lizibilitate)
        ...
        if (cur != null) {
          ...
          if (dist < 10) {
            // Èšintim o randare deasupra (NORD) pentru casÄƒ faÈ›Äƒ de maÈ™inÄƒ
            lat += 0.00006; 
          }
        }
        ...
        if (_mapOrientationPinManager != null) {
          await _mapOrientationPinManager!.create(
            PointAnnotationOptions(
              ...
              iconSize: 0.85,
              iconAnchor: IconAnchor.BOTTOM,
              symbolSortKey: 2e6, // âœ… Deasupra tuturor (È™oferi, emoji, POI)
            ),
          );
          Logger.debug('Home Pin: Created OK (Asset bytes: ${bytes.length})', tag: 'MAP_HOME');
        }
      } catch (e, st) {
        Logger.error('Reper orientare (pin AcasÄƒ): $e', tag: 'MAP_HOME', error: e, stackTrace: st);
      } finally {
  ...
  Future<void> _onMapCreated(MapboxMap mapboxMap) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (_currentPositionObject != null) {
          unawaited(_updateWeatherAndStyle(_currentPositionObject!.latitude, _currentPositionObject!.longitude));
          unawaited(_updateUserMarker(centerCamera: false));
          unawaited(_updateLocationPuck());
        }
  ...
  Future<void> _onStyleLoaded(StyleLoadedEventData event) async {
    ...
    try {
      ...
      // Refresh manageri annotations
      if (mounted) {
        unawaited(_requestsManager?.initialize());
        unawaited(_mysteryBoxManager?.initialize());
        unawaited(_communityMysteryManager?.initialize());
        _mapOrientationPinManager = null; // ✅ Forțează recrearea managerului pe noul stil
        _userPointAnnotationManager = null; // ✅ Asigură că avatarul propriu este recreat
        _userPointAnnotation = null; // ✅ Resetăm obiectul de adnotare pentru a evita mismatch manager pe noul stil
        unawaited(_syncMapOrientationPinAnnotation());
        if (_currentPositionObject != null) {
          unawaited(_updateUserMarker(centerCamera: false));
          unawaited(_updateLocationPuck());
  ...
  static const double _driverMarkerPosLerp = 0.34;
  static const double _driverMarkerBearingLerp = 0.28;
  ...
  Future<void> _runUserMarkerUpdate({bool centerCamera = false}) async {
    ...
    try {
      // Ierarhie: Galaxy Garage (non-default) → șofer disponibil fără garaj = berlină standard app → emoji profil (pasager / off-duty).
      ...
      _driverMarkerSmoothLat = _driverMarkerSmoothLat! +
          (tgtLat - _driverMarkerSmoothLat!) * _driverMarkerPosLerp;
      _driverMarkerSmoothLng = _driverMarkerSmoothLng! +
          (tgtLng - _driverMarkerSmoothLng!) * _driverMarkerPosLerp;

      final moving = headingSrc.speed >= 0.5;
      ...
      _driverMarkerSmoothHeadingDeg = _lerpBearingDegrees(
        ...
        headingForIcon,
        moving ? _driverMarkerBearingLerp : 0.12,
      );
      ...
      if (_userPointAnnotation == null) {
        ...
        } else if (_driverOnDutyOnMap) {
          bitmapFuture =
              _generateMarkerIcon(isPassenger: false, customAssetPath: null);
        } else {
          bitmapFuture = NeighborFriendMarkerIcons.buildEmojiMarker(
            emoji: _nametagEmojiFromProfile(),
            isOnline: true,
          );
        }
        ...
        final options = PointAnnotationOptions(
          ...
          symbolSortKey: 1.5e6, // Ô£à Ierarhie: Home(2e6) > Car(1.5e6) > Mystery(1e6)
          textField: textField,
  ...
  Future<void> _updateLocationPuck() async {
    ...
    try {
      ...
      Logger.warning('Could not update location puck: $e');
    }
  }
  ...
  Future<void> _initializeScreen({bool useCommittedLocalRole = false}) async {
    try {
      ...
      _savedAddressesForHomePinSub =
        ...
        if (!mounted) return;
        setState(() => _savedAddressesForHomePin = list);
        unawaited(_syncMapOrientationPinAnnotation());
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) unawaited(_syncMapOrientationPinAnnotation());
        });
                  ...
                  _buildBumpFloatingButton(
                    ...
                    onTap: () async {
                      if (!_isRadarMode && _mapboxMap != null) {
                         // Inițializăm centrul radarului la centrul camerei
                         final cam = await _mapboxMap!.getCameraState();
                      ...
                      if (_isRadarMode) {
                        _showSafeSnackBar('RADAR ACTIVAT 🕸️. Aruncă plasa peste grupuri!', const Color(0xFF7C3AED));
                      }
                    },
                  ),
                  const SizedBox(height: 12),
